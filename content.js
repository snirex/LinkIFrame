javascript:(function () {
    ...
    // מוסיף האזנה לכל הקישורים – פתיחת iframe אחרי 3 שניות ריחוף
    document.querySelectorAll("a").forEach(link => {
        link.addEventListener("mouseenter", () => {
            hoverTimer = setTimeout(() => {
                showIframe(link.href);
            }, 3000);
        });
    ...
    document.addEventListener("keydown", e => {
        if (e.key === "Escape" && windowStack.length) {
            ...
            topWindow.div.style.transform = "translate(+50%, +50%) scale(0.3)";
            setTimeout(() => {
                topWindow.div.remove();
                windowStack = windowStack.filter(w => w.div !== topWindow.div);
            }, 300);
        }
    });

    /** פונקציה ליצירת חלון iframe */
    function showIframe(url) {
        const previewBox = document.createElement("div");
        previewBox.style.position = "fixed";
        previewBox.style.top = (50 + windowStack.length * 20) + "%";
        previewBox.style.left = (50 + windowStack.length * 20) + "%";
        ...
        titleSpan.addEventListener("keydown", e => {
            if (e.key === "Enter")
                searchInIframe(previewBox.querySelector("iframe"), titleSpan.value);
        });
        ...
        const closeBtn = makeBtn("✕", "סגור", () => {
            previewBox.style.opacity = "0";
            previewBox.style.transform = "translate(+50%, +50%) scale(0.3)";
            setTimeout(() => {
                previewBox.remove();
                windowStack = windowStack.filter(w => w.div !== previewBox);
            }, 100);
        });

        let minimized = false, originalHeight = "500px";
        const minimizeBtn = makeBtn("−", "צמצם/הגדל", () => {
            if (!minimized) {
                originalHeight = previewBox.style.height;
        ...
        const openInPageBtn = makeBtn("💻", "פתח בדף הראשי", () => {//⬈
            const iframeTag = previewBox.querySelector("iframe");
            if (iframeTag && iframeTag.src)
                window.location.href = iframeTag.src;
        });

        const refreshBtn = makeBtn("⟳", "רענן", () => {
            const iframeTag = previewBox.querySelector("iframe");
            if (iframeTag) iframeTag.src = iframeTag.src;
        });

        const forwardBtn = makeBtn("→", "קדימה", () => {
            const iframeTag = previewBox.querySelector("iframe");
            if (iframeTag) iframeTag.contentWindow.history.forward();
        });

        const backBtn = makeBtn("←", "אחורה", () => {
            const iframeTag = previewBox.querySelector("iframe");
            if (iframeTag) iframeTag.contentWindow.history.back();
        });
        ...
        const fullBtn = makeBtn("⛶", "מסך מלא", () => {
            if (!isFull) {
                ...
                previewBox.dataset.prevTransform = previewBox.style.transform;
                ...
                previewBox.style.bottom = "0";
                previewBox.style.transform = "translate(-50%, 0) scale(1)";
                isFull = false;
            }
        });

        const copyBtn = makeBtn("📋", "העתק URL", () => {
            const iframeTag = previewBox.querySelector("iframe");
            if (iframeTag) navigator.clipboard.writeText(iframeTag.src);
        });

        // הוספת הכפתורים ל־header
        ...
        iframe.addEventListener("load", () => {
            try {
                titleSpan.value = iframe.contentDocument.title || "ללא כותרת";
            } catch (e) {
                titleSpan.value = "ללא כותרת";
            }
        });
    ...
    function makeDraggable(element, handle) {
        ...
        document.addEventListener("mouseup", () => {
            if (isDown) element.style.transition = "all 0.3s ease";
            isDown = false;
    ...
    function makeResizable(element, resizer, iframe) {
        ...
        document.addEventListener("mouseup", () => {
            if (isResizing) element.style.transition = "all 0.3s ease";
            isResizing = false;
            document.body.style.userSelect = "auto";
        });
    }
